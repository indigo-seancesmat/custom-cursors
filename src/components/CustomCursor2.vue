...
export default {
  ...
  methods: {
    ...
    setUpCanvas() {
      ...
      this.polygon = new paper.Path.Rectangle(
        new paper.Point(0, 0),
        // segments,
        new paper.Size(15, 15)
      );
      this.polygon.strokeColor = strokeColor;
      this.polygon.strokeWidth = strokeWidth;
      this.polygon.smooth({ type: "continuous", from: 0, to: 0 });
    ...
    setUpHoverState() {
      ...
      const handleMouseEnter = e => {
        ...
        gsap.to(this.shapeBounds, { width: this.navItemBox.width });
        gsap.to(this.shapeBounds, { height: this.navItemBox.height });
      ...
      const handleMouseLeave = () => {
        this.isStuck = false;
        // gsap.to(this.shapeBounds, { width: 30 });
        // gsap.to(this.shapeBounds, { height: 30 });
      };
    ...
    setUpMagnet() {
      ...
      paper.view.onFrame = event => {
        ...
        this.polygon.bounds.height = this.shapeBounds.height;
        ...
        //   //   this.polygon.bounds.width = this.shapeBounds.width;
        //   //   this.polygon.bounds.height = this.shapeBounds.height;
        //   // this.polygon.size = new paper.
        // } else
        if (!this.isStuck && this.polygon.bounds.width > 30) {
          // remove noise
          if (this.isNoisy) {
            this.polygon.segments.forEach((segment, i) => {
              if (this.bigCoordinates[i]) {
                segment.point.set(
          ...
          //   gsap.to(this.shapeBounds, 1, { width: 30 });
          //   gsap.to(this.shapeBounds, 1, { height: 30 });
          //   const scaleDown = 0.92;
          //   this.polygon.scale(scaleDown);
        }

        if (this.isNoisy === true) {
          this.addNoise(event);
        }
        this.polygon.smooth({ type: "continuous", from: 0, to: 0 });
      };
